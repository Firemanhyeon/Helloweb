...
//로그인메소드2
	...
	public boolean modify(BoardVo number) {
		sql = "update tbl_Board set brd_title=nvl(?,brd_title), brd_content = nvl(?,brd_content)"
				+" where brd_no = ?";
		conn = Dao.getConnect();
		try {
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, number.getTitle());
			psmt.setString(2, number.getContent());
			psmt.setInt(3, number.getNumber());
